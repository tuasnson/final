...
import com.se2_project.group12.demoEBanking.IService.IBillService;
		...
		if (serviceRepository.existsById(bil.getService().getServiceId())) {
			...
			if (!billRepository.existsById(bil.getTransactionId())) {
				if (checkValidate.getOk(bil)) {
					...
					com.se2_project.group12.demoEBanking.Model.Service sv = serviceRepository.findById(bil.getService().getServiceId()).get();
